...
function setupTriggers() {
  Promise.all([setupSideBar(), setupProfile(), setupProfilePhoto()]).catch(console.error);
}
...
async function setupProfile() {
  ...
  if (user.username === profile_user || user.role === "admin") {
    ...
    document.getElementById("boton-perfil-template-profile-photo").classList.add("hidden");
  }
}
